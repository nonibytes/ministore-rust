...
pass "keyword prefix match works (wildcard)"

# has: predicate (must be combined with positive anchor)
RESULT=$($CLI search --index $INDEX --where 'programming AND has:title' --format json | jq -r '.items | length')
[ "$RESULT" -ge 2 ] || fail "programming AND has:title should return at least 2 items, got $RESULT"
pass "has: predicate works (with positive anchor)"

# bool predicate (must be combined with positive anchor)
RESULT=$($CLI search --index $INDEX --where 'programming AND active:true' --format json | jq -r '.items | length')
[ "$RESULT" -eq 2 ] || fail "programming AND active:true should return 2 items, got $RESULT"
pass "bool predicate works (with positive anchor)"
...
# NOT query
RESULT=$($CLI search --index $INDEX --where 'programming AND NOT active:false' --format json | jq -r '.items | length')
...
# Recency ranking
FIRST_PATH=$($CLI search --index $INDEX --where 'programming' --rank recency --format json | jq -r '.items[0].path')
[ -n "$FIRST_PATH" ] || fail "Recency ranking returned no results"
pass "Recency ranking works"

# Field ranking (requires Patch 4)
FIRST_PATH=$($CLI search --index $INDEX --where 'category:technical' --rank 'field:score' --format json | jq -r '.items[0].path')
...
# Discover fields
FIELD_COUNT=$($CLI discover fields --index $INDEX | jq 'length')
[ "$FIELD_COUNT" -ge 9 ] || fail "discover fields should show at least 9 fields, got $FIELD_COUNT"
pass "Discover fields works"

# Discover values
TAG_COUNT=$($CLI discover values --index $INDEX --field tags | jq 'length')
...
RESULT=$($CLI discover values --index $INDEX --field tags --where 'active:true' | jq 'length')
...
# Verify deletion by searching for items that should be gone
RESULT=$($CLI search --index $INDEX --where 'category:notes' --format json | jq -r '.items | length')
...
RESULT=$($CLI search --index $INDEX --where 'programming' --limit 2 --format json)
